...
/**
 * The {@code TimeObjectMapping} class represents a mapping from integers to arbitrary objects. Values of this mapping's
 * domain are referred to as time henceforth, since time being the domain will be the primary application for such
 * mappings. Internally, the {@code TimeObjectMapping} is considered as a step function. Consequently, the mapping is
 * stored as a sorted collection of step starts which is obviously sufficient to encode the mapping. The size needed to
 * encode an {@code TimeObjectMapping} is therefore linear in the number of steps required. In order to access steps
 * efficiently, a TreeSet is used which in turn is based on a red-black tree. This allows the addition, removal and
 * search for steps in O(log (number of steps)) time. For mappings of integers to integers see
...
public class TimeObjectMapping<R> implements Cloneable, Iterable<TimeObjectPair<R>>, Mapping<Integer, R> {
    ...
    /**
     * Creates a new {@code IntegerObjectMapping} that is defined for all integer values. Initially, all integers are
     * mapped to null. Runtime O(1).
     */
    ...
    /**
     * Returns the value associated with the specified integer. Runtime O(log (number of steps)).
     *
    ...
    /**
     * Maps the integer {@code time} to the object {@code value}. Runtime O(log (number of steps)).
     *
    ...
    /**
     * Returns an iterator over the time - value mappings in this {@code TimeObjectMapping}. Runtime O(1).
     *
     * @return an iterator over the time - value mappings in this {@code TimeObjectMapping}.
     */
    @Override
     ...
     * object is not {@code null</code}, of type {@code TimeObjectMapping} and makes exactly the same time - value
     * associations. Runtime O(number of steps).
     *
